...
import cerec.biblio.demo.dto.TokenRefreshRequest;
import cerec.biblio.demo.exceptions.TokenRefreshException;
import cerec.biblio.demo.exceptions.domain.*;
import cerec.biblio.demo.model.HttpResponse;
import cerec.biblio.demo.model.RefreshToken;
import cerec.biblio.demo.model.User;

import cerec.biblio.demo.model.UserPrincipal;
import cerec.biblio.demo.security.service.RefreshTokenService;
import cerec.biblio.demo.security.service.UserService;
import cerec.biblio.demo.security.utility.JWTTokenProvider;
...
import static cerec.biblio.demo.Constant.FileConstant.*;
import static cerec.biblio.demo.Constant.SecurityConstant.*;


@RestController
@RequestMapping(value = "/user")
public class UserController {
    public static final String EMAIL_SENT = "Un email avec un nouveau mot de passe a été envoyé à: ";
    public static final String USER_DELETED_SUCCESSFULLY = "Suppression de l'utilisateur reussie";
    ...
    @Autowired
    public UserController(AuthenticationManager authenticationManager, UserService userService, JWTTokenProvider jwtTokenProvider, RefreshTokenService tokenService) {
    ...
    public ResponseEntity<User> register(@RequestBody User user) throws UserNotFoundException, UsernameExistException, EmailExistException, MessagingException {
        User newUser = userService.register(user.getUsername(),user.getNom(),user.getPrenom(), user.getEmail(),user.getPassword());
    ...
    public ResponseEntity<User> refreshtoken(@Valid @RequestBody TokenRefreshRequest request) {
        HttpHeaders headers = new HttpHeaders();
        String requestRefreshToken = request.getRefreshToken();
        RefreshToken refreshToken=tokenService.findByToken(requestRefreshToken).orElseThrow(() -> new TokenRefreshException(requestRefreshToken, "Refresh token is not in database!"));
        ...
        headers.add(REFRESH_TOKEN,refreshToken.getToken());


        return new ResponseEntity<>(user,headers,OK);
    }
